...
def main() -> None:
    # All heavy work lives here, behind the __main__ guard: vLLM spawns
    # EngineCore subprocesses which re-import this module, so module-level
    # LLM-booting code would recursively spawn (the "bootstrapping phase"
    # RuntimeError). Imports stay at module top; execution is guarded.
    import torch
    ...
    instances = [
        json.loads(l) for l in DATA.read_text().splitlines() if l.strip()
    ][:1]  # one instance is enough to inspect metrics
    ...
    # Free the HF encoder (~15GB for an 8B model) BEFORE booting vLLM, or
    # both 8B models sit in VRAM at once and OOM a 24GB card.
    ...
    with _pc_test_bind_storage(index.kv_store):
        ...
        llm.generate(
            [TokensPrompt(prompt_token_ids=list(prod_ids[:32]))],
    ...
    print(f"type: {type(m)}")
    if m is None:
        print("!!! metrics is None — disable_log_stats=False did NOT "
              "populate it")
    else:
        for f in dataclasses.fields(m):
            print(f"  {f.name} = {getattr(m, f.name)}")
        print("\n--- candidate TTFT sources ---")
        for attr in ("prefill_time", "first_token_latency",
                     "inference_time", "decode_time", "e2e_latency"):
            print(f"  {attr} = {getattr(m, attr, 'MISSING')}")
